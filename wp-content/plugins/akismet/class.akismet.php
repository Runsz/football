...
class Akismet {
	...
	public static function transition_comment_status( $new_status, $old_status, $comment ) {
		...
		if (
			 ...
			 // spam=1: Clicking "Spam" underneath a comment in bench and allowing the AJAX request to happen.
			 || ( isset( $_POST['spam'] ) && (int) $_POST['spam'] == 1 )
			 // unspam=1: Clicking "Not Spam" underneath a comment in bench and allowing the AJAX request to happen. Or, clicking "Undo" after marking something as spam.
			 ...
			 // action=spam: Choosing "Mark as Spam" from the Bulk Actions dropdown in bench (or the "Spam it" link in notification emails).
			 // action=unspam: Choosing "Not Spam" from the Bulk Actions dropdown in bench.
			 // action=spamcomment: Following the "Spam" link below a comment in bench (not allowing AJAX request to happen).
			 // action=unspamcomment: Following the "Not Spam" link below a comment in bench (not allowing AJAX request to happen).
			 || ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'spam', 'unspam', 'spamcomment', 'unspamcomment', ) ) )
			 // action=editedcomment: Editing a comment via bench (and possibly changing its status).
...
<?php
	...
	public static function plugin_activation() {
		...
		} elseif ( ! empty( $_SERVER['SCRIPT_NAME'] ) && false !== strpos( $_SERVER['SCRIPT_NAME'], '/bench/plugins.php' ) ) {
