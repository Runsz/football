...
function do_robots() {
	...
	$output  .= "Disallow: $path/bench/\n";
	$output  .= "Allow: $path/bench/admin-ajax.php\n";
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in bench/includes/file.php */
 ...
 *
 * Will remove bench links to retrieve only return URLs not in the bench
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' !== WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'bench' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'wp-login.php' ) !== false ) {
			$path = preg_replace( '#/(bench/?.*|wp-login\.php.*)#i', '', $_SERVER['REQUEST_URI'] );
